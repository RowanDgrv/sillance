...
function openStravaAnalysis(act){
  ...
  PF.getActivityStreams(act.id).then(({points, laps})=>{
    ...
    const ftp=(typeof ATHLETE_REF!=='undefined'&&ATHLETE_REF&&ATHLETE_REF.ftp)||270;
    // Vrais laps de la montre (bouton lap) si l'athlète en a posé, sinon []
    // → PFFit retombe sur son découpage automatique au km (comportement
    // historique). Avant ce correctif (23/09/2026), toujours [] : l'app
    // recalculait des splits au km même pour des intervalles lappés à la main
    // (ex. 15x30s actif) — illisible pour analyser la séance réellement faite.
...
function injectLapCss(){
  ...
  /* Tableau façon Coros (23/09/2026, remplace les cartes du 25/08 : sur une
     séance fractionnée à beaucoup de laps courts — ex. 15x30s — un tableau
     dense se scanne bien plus vite qu'une grille de cartes, et se rapproche
     de ce que montrent Coros/Strava natifs). Numéro de tour + étiquette
     Échauffement/Course/Retour au calme (heuristique par allure relative,
     seulement sur de VRAIS laps — cf. data.realLaps), temps de lap ET temps
     cumulé, tour le plus rapide surligné comme "Meilleur tour". */
...
/* Étiquette Échauffement/Course/Retour au calme par lap — heuristique par
   allure relative à la médiane de la séance (Strava/Coros n'exposent pas le
   type d'intervalle d'origine via leur API publique) : le 1er/dernier lap
   nettement plus long ET pas plus rapide que la médiane = échauf./retour au
   calme, sinon plus rapide que la médiane = travail, plus lent = récup.
   N'est tenté que sur de VRAIS laps (data.realLaps) — un auto-split au km
   n'a pas cette structure et resterait un classement aléatoire. */
