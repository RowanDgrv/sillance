...
window.__pf_app = {
  ...
  // Redessine l'indice de forme + le panneau "aujourd'hui" — à appeler après
  // toute bascule d'athlète ou chargement de planning réel (les signaux ACWR/
  // TSB/RPE sont recalculés depuis le calendrier fraîchement chargé).
  refreshForm(){
...
const READINESS = {
  ...
  // ACWR = charge aiguë 7j / chronique 28j de la semaine en cours.
  get acwr(){ const w = this._weeks; return w ? Math.round(w[w.length-1].acwr*100)/100 : 1; },
  // TSB (forme = CTL - ATL) de la semaine en cours.
  ...
  // RPE des 3 dernières séances VALIDÉES (≤ 14 j). Neutre (6) si rien de récent.
  get rpeRecent(){
    const out = [];
